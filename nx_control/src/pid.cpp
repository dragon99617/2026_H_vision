...
PidResult BallPid::calculate(const ObserverState& estimate,
  ...
  result.proportional_m_s2 = config_.pid_kp_s2 * result.position_error_m;
  result.integral_m_s2 = integral_output_m_s2_;
  result.derivative_m_s2 = config_.pid_kd_s_inv * result.velocity_error_m_s;
  ...
  result.disturbance_m_s2 =
      -config_.pid_disturbance_gain * estimate.disturbance_m_s2 /
...
PidResult BallPid::track(double applied_m_s2, bool force_freeze) {
  ...
  if (last_result_.saturated || force_freeze) {
    bool limited = false;
    integral_output_m_s2_ = clamp_integral(
