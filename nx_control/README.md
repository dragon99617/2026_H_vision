...
- 对需要底盘状态的通用/兼容任务提供实际加速度低延迟滤波和当前加速度前馈；
  赛题任务4/5/6不读取底盘运动状态，而是按已知的4.5秒、0→70 RPM五次启动
  曲线和6.5 cm车轮结构生成确定性加速度前馈；
...
```text
s = clamp(t / 4.5, 0, 1)
n_ref = 70 * (10*s^3 - 15*s^4 + 6*s^5)                  [RPM]
a_chassis = (2*pi*r/60) * (70/4.5) * 30*s^2*(1-s)^2   [m/s^2]
theta_ff = atan(0.85*a_chassis/g)                       [rad]
```

其中车轮半径 `r=0.0325 m`，默认认为转速已经是车轮端RPM（传动换算为1:1）。
理论加速度峰值约为`0.0993 m/s²`、理论全补偿角约为`0.580°`；当前85%保守
前馈的峰值角约为`0.493°`，出现在`2.25 s`。启动段冻结积分并把P/D/扰动修正
缩放到15%；`t>=4.5 s`时启动加速度和前馈角归零，恢复100%普通PID。观测器仍
使用未缩放的完整理论底盘加速度，避免把刻意保留的15%前馈余量误认为模型本身。
...
- `--task 6 --target-cm N`：赛题要求6，车辆行驶阶段保持指定位置 `N` cm，
  同样按4.5秒五次曲线从0加速到70 RPM，控制输入与任务4/5相同；
...
明确的`--task 3`静止任务，以及视觉闭环的`--task 4`、`--task 5`、`--task 6`，
允许在缺少新鲜`chassis-state-v1`时运行。其中任务4/5/6只在启动4.5秒内使用
本地五次曲线的理论加速度，之后预测域置零；收到的底盘报文只记录而不参与控制。
其他任务（包括旧`--task static`）仍要求底盘状态新鲜且无故障。
...
# 要求4：视觉位置闭环，4.5秒加速到70 RPM并保持中心
./build/ball_nx_control --config config/nx-control.conf --task 4

# 要求5：启动曲线与要求4相同
./build/ball_nx_control --config config/nx-control.conf --task 5
...
会忽略该参数。四个赛题 task 只负责摆杆滚球控制，小车循迹、AB/整圈计时和停车
仍由底盘控制端负责。
