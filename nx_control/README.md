...
- 管角硬限幅、角速度限制、内环角差减速监控和轻量边界预测；视觉丢失100～250 ms
  先进入0° HOLD软保护，超过250 ms进入锁存SAFE；DMMC状态缺失、超时或故障进入
  锁存Fault。底盘状态、内环角差和位置边界均不触发NX安全锁存；
...
0.2秒时请求底盘减速，但不触发SAFE锁存。驻留时间使用NX单调时钟的真实持续时间
计算，不使用固定样本计数，也不要求DMMC02以1 kHz回传状态。该监控不改变机构
逆解、拟合、PID或摩擦参数。
